...
def 模拟1次(
    ...
    if 初始订单 is None:
        初始订单 = random.choices(跑单时的贸易站.订单列表,
                              [订单.概率 * 订单.需要秒基础工时 for 订单 in 跑单时的贸易站.订单列表])[0]
        # 初始订单 = 能跑单时的贸易站.生成订单(0)
    if 初始订单进度 is None:
    ...
    本来能获得龙门币: float = (
        初始订单.获得龙门币 * (1 - 初始订单进度)
        + 跑单时的贸易站.每秒基础工时获得龙门币 * (当前时间 * 跑单时的效率 - 初始订单.需要秒基础工时 * (1 - 初始订单进度))
    )
    本来能消耗赤金: float = (
        初始订单.消耗赤金 * (1 - 初始订单进度)
        + 跑单时的贸易站.每秒基础工时消耗赤金 * (当前时间 * 跑单时的效率 - 初始订单.需要秒基础工时 * (1 - 初始订单进度))
    )
...
def 模拟n次(
    ...
    多获得龙门币标准差: float = statistics.stdev(多获得龙门币列表)
    多消耗赤金标准差: float = statistics.stdev(多消耗赤金列表)
...
def main():
    ...
    for plan_name, plan in 对比.items():
        ...
        for 贸易站等级, 贸易站情况, 初始订单, 初始订单进度, 跑单时的效率字符串 in zip(
            ...
            跑单时的贸易站: 贸易站数据 = 贸易站数据.new(贸易站等级=贸易站等级, 但书=True, 龙舌兰=True, 裁缝=None)
            多获得龙门币, 多消耗赤金, 多获得龙门币标准差, 多消耗赤金标准差 = 模拟n次(
                模拟次数=模拟次数,
                无法跑单的时间=无法跑单的时间,
                跑单时的贸易站=跑单时的贸易站,
                跑单时的效率=parse_percentage(跑单时的效率字符串),
                无法跑单时的贸易站=贸易站数据.new(
                    贸易站等级,
